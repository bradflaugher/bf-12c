 ...
 * [CalculatorTest]; `g n` is 12×, `f n` AMORT, `f yx` PRICE, `g CHS` DATE, etc.
 */
class HandbookTest {
    ...
    private fun Calculator.keys(script: String): Calculator {
        for (token in script.trim().split(Regex("\\s+"))) {
            names[token]?.let { press(it) } ?: token.forEach { c -> press(if (c == '.') Key.DOT else Key.digit(c - '0')) }
        }
        return this
    }
    ...
    /** Equal to within half a unit in the last printed digit. */
    private fun near(expected: String, actual: BigDecimal) {
    ...
    @Test fun tvm() {
        near("10470.8497", run("14 ENTER 2 * n 5.35 ENTER 2 / i 5000 CHS PV FV").x)
        near("-21396.6097", run("4 g n 6 g i 500 PMT g 7 PV").x)
        near("-41.65292", run("14 g n 6 g i 10925.76 FV g 8 PMT").x)
        near("0.41754", run("14 g n 6 g i 10925.76 FV 45 CHS PMT i").x)
        near("1.60914", run("8 ENTER 4 * n 6000 CHS PV 10000 FV i").x)
        near("5389.7221", run("4 g n 15 g i 150 CHS PMT g 8 PV").x)
        near("-523.98835", run("29 g n 14.25 g i 43400 PV g 8 PMT").x)
        near("-42652.3689", run("5 g n 14.25 g i 43400 PV 523.99 CHS PMT g 8 FV").x)
        near("1281.3359", run("2 g n 6.25 g i 50 CHS PMT g 7 FV").x)
        near("-717.44030", run("15 ENTER 2 * n 9.75 ENTER 2 / i 3200 CHS PV 60000 FV g 8 PMT").x)
        near("28346.9562", run("6 n 2 CHS i 32000 CHS PV FV").x)
    }

    @Test fun periodsRoundUpThenFinalPayment() {
        val c = run("10.5 g i 35000 PV 325 CHS PMT g 8 n")
        near("328", c.x)
        near("181.89", c.keys("328 n FV").x)
        near("-141.87", run("10.5 g i 35000 PV 325 CHS PMT 327 n FV").x)
    }

    @Test fun cashFlows() {
        val duplex = "80000 CHS g PV 500 CHS g PMT 4500 g PMT 5500 g PMT 4500 g PMT 130000 g PMT"
        near("212.18405", run("$duplex 13 i f PV").x)
        near("13.06289", run("$duplex f FV").x)
        ...
        near("907.76893", run("$grouped 13.5 i f PV").x)
        near("13.71972", run("$grouped f FV").x)
    }

    @Test fun simpleInterest() {
        val c = run("60 n 7 i 450 CHS PV f i")
        near("5.25", c.x)
        near("5.17808", c.stack[2])
        near("455.25", c.keys("+").x)
        val d = run("45 n 9.75 i 750 CHS PV f i")
        near("9.140625", d.x)
        near("9.01541", d.stack[2])
    }

    @Test fun depreciation() {
        val sl = run("50000 PV 8000 FV 6 n 1 f %T")
        near("7000", sl.x)
        near("35000", sl.stack[1])
        near("12000", run("50000 PV 8000 FV 6 n 1 f D%").x)
        near("4000", run("50000 PV 8000 FV 6 n 5 f D%").x)
        val db = "10000 PV 500 FV 5 n 200 i"
        near("4000", run("$db 1 f %").x)
        near("2400", run("$db 2 f %").x)
        val y3 = run("$db 3 f %")
        near("1440", y3.x)
        near("1660", y3.stack[1])
    }

    @Test fun amortization() {
        val c = run("f 2 13.25 g i 50000 PV 573.35 CHS PMT 12 f n")
        near("-6608.89", c.x)
        near("-271.31", c.stack[1])
        near("49728.69", c.fin[2])
        c.keys("12 f n")
        near("-6570.72", c.x)
        near("-309.48", c.stack[1])
        near("49419.21", c.fin[2])

        val m = run("f 2 30 g n 13.25 g i 50000 PV g 8 PMT")
        near("-562.88676", m.x)
        m.keys("0 n 1 f n")
        near("-552.08", m.x)
        near("-10.81", m.stack[1])
        near("49989.19", m.fin[2])
    }
    ...
    @Test fun bonds() {
        ...
        near("87.6218", p.x)
        near("2.6889", p.stack[1])
        near("8.1506", run("88.375 PV 6.75 PMT 4.281982 ENTER 6.041996 f 1/x").x)
        val onCoupon = run("7 i 6 PMT 1.012000 ENTER 1.012005 f yx")
        near("95.8417", onCoupon.x)
        near("0", onCoupon.stack[1])
    }
    ...
    @Test fun statistics() {
        ...
        near("21714.29", mean.x)
        near("40.00", mean.stack[1])
        val s = run("$data g .")
        near("4820.5908", s.x)
        near("6.0277", s.stack[1])
        val est = run("$data 48 g 1")
        near("28818.9263", est.x)
        near("0.90052", est.stack[1])
        near("15.54918", run("$data 0 g 2").x)
        near("1.18653", run("1.16 ENTER 15 S+ 1.24 ENTER 7 S+ 1.2 ENTER 10 S+ 1.18 ENTER 17 S+ g 6").x)
    }

    @Test fun percentages() {
        near("42", run("300 ENTER 14 %").x)
        near("12921.40", run("13250 ENTER 8 % - 6 % +").x)
        near("-8.97436", run("58.5 ENTER 53.25 D%").x)
        val t = run("3.92 ENTER 2.36 + 1.67 + 2.36 %T")
        near("29.69", t.x)
        near("7.95", t.stack[1])
        near("49.31", t.keys("CLX 3.92 %T").x)
    }
}
